...
/** Full explainer: diagram, cards, framing — shown inside the modal. */
export function CdpExplainerContent() {
  return (
    <div className="space-y-4 text-left">
      <div className="rounded-lg border border-amber-900/45 bg-amber-950/30 px-3 py-2.5" role="region" aria-label="Time-of-ingestion summary">
        ...
        <p className="text-[10px] text-amber-200/65 mt-1.5">
          Turn on <span className="font-semibold text-amber-100/90">Ingest-time Tealium</span> in the left nav to re-model Tealium steps that way. Events table still shows <span className="font-semibold">Ingest semantics</span> per row.
        </p>
      </div>

      <div className="border-b border-gray-700 pb-2">
        <p className="text-[10px] font-bold uppercase tracking-widest text-cyan-400/90">Tealium CDP → CJA</p>
        <p className="text-[10px] text-gray-500 mt-0.5 leading-snug">Diagram and constraints below.</p>
      </div>
...
function IngestSnapshotDiagram() {
  const markerId = `cdp-arrow-${useId().replace(/[^a-zA-Z0-9_-]/g, '')}`;
  ...
  return (
    <div className="rounded-lg bg-gray-950/90 border border-gray-700/80 p-3 overflow-x-auto">
      ...
      <svg viewBox="0 0 420 88" className="w-full min-w-[280px] h-[72px]" aria-label="Tealium resolves identity over time but AEP rows stay fixed at ingest unless replay or downstream join">
        ...
        <line x1="68" y1="48" x2="68" y2="58" stroke="#64748b" strokeWidth="1.5" markerEnd={`url(#${markerId})`} />
        <line x1="192" y1="48" x2="192" y2="58" stroke="#64748b" strokeWidth="1.5" markerEnd={`url(#${markerId})`} />
